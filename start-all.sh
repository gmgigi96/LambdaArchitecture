...
aws emr put --cluster-id $CLUSTER_ID --key-pair-file $PRI_KEY --src deploy.sh --dest /home/hadoop/
aws emr put --cluster-id $CLUSTER_ID --key-pair-file $PRI_KEY --src config --dest /home/hadoop/

aws emr put --cluster-id $CLUSTER_ID --key-pair-file $PRI_KEY --src posts.xml --dest /home/hadoop/
aws emr put --cluster-id $CLUSTER_ID --key-pair-file $PRI_KEY --src src --dest /home/hadoop/

aws emr put --cluster-id $CLUSTER_ID --key-pair-file $PRI_KEY --src start-simulation.sh --dest /home/hadoop/

# permessi a deploy.sh
aws emr ssh --cluster-id $CLUSTER_ID --key-pair-file $PRI_KEY --command "chmod +x deploy.sh"
